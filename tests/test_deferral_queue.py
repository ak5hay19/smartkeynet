...
def test_higher_sensitivity_class_served_before_lower():
    queue = DeferralQueue()
    queue.enqueue(make_request("low", sensitivity_class=1, step=0), bits_required=100.0, step=0, pool_fill_at_onset=0.0)
    queue.enqueue(make_request("high", sensitivity_class=3, step=1), bits_required=100.0, step=1, pool_fill_at_onset=0.0)
...
def test_fifo_ordering_within_same_class():
    ...
    for i, rid in enumerate(["r0", "r1", "r2"]):
        queue.enqueue(make_request(rid, sensitivity_class=2, step=i), bits_required=50.0, step=i, pool_fill_at_onset=0.0)
...
def test_pop_servable_respects_cumulative_pool_headroom():
    ...
    queue = DeferralQueue()
    queue.enqueue(make_request("a", sensitivity_class=1, step=0), bits_required=80.0, step=0, pool_fill_at_onset=0.0)
    queue.enqueue(make_request("b", sensitivity_class=1, step=1), bits_required=80.0, step=1, pool_fill_at_onset=0.0)
...
def test_pop_servable_skips_unfit_head_of_line_for_smaller_lower_priority():
    ...
    queue = DeferralQueue()
    queue.enqueue(make_request("big-high", sensitivity_class=3, step=0), bits_required=1000.0, step=0, pool_fill_at_onset=0.0)
    queue.enqueue(make_request("small-low", sensitivity_class=1, step=1), bits_required=10.0, step=1, pool_fill_at_onset=0.0)
...
def test_regret_event_fires_once_on_enqueue_not_on_every_tick():
    queue = DeferralQueue()
    event = queue.enqueue(make_request("r0", sensitivity_class=2, step=5), bits_required=100.0, step=5, pool_fill_at_onset=42.0)
...
def test_tick_returns_one_entry_per_still_queued_request():
    queue = DeferralQueue()
    queue.enqueue(make_request("r0", sensitivity_class=1, step=0), bits_required=10.0, step=0, pool_fill_at_onset=0.0)
    queue.enqueue(make_request("r1", sensitivity_class=2, step=0), bits_required=10.0, step=0, pool_fill_at_onset=0.0)
...
def test_deferred_request_served_once_pool_can_cover_it():
    queue = DeferralQueue()
    queue.enqueue(make_request("r0", sensitivity_class=3, step=0), bits_required=500.0, step=0, pool_fill_at_onset=0.0)
