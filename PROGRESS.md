...
The reward-weighting-vs-scenario-triviality question (below) is now
**resolved, directionally**: build the real S4 scenario next.

2026-08-10's load-spike diagnostic (see SESSION_LOG.md) tested the
open question directly, without waiting for the real tenant graph: it
gave `random_request_generator` an optional, config-driven, *periodic*
arrival-rate spike (explicitly not real S4 — see below) and re-ran the
same S1 training+comparison under it. Result: `forced_rekey_ratio`
dropped from flat S1's exact `1.000` (never proactive, both this
session's control run and the prior session's) to `0.256` and `0.872`
across two training seeds — proactive rekeying does emerge once
arrival load genuinely varies over time. This supports the read that
`reward.w_fr`/`reward.c_rekey_base` are **not** structurally broken and
S1's stationarity was the real reason nothing proactive ever showed up
there — **no `reward.*` change is being made or requested** on this
evidence. `experiments/harness.py`'s `ScenarioResult` also gained
`total_reward` this session (the second half of the prior "next task"
item) — a much sharper policy discriminator than `p99_latency` turned
out to be (e.g. a ~140x total_reward gap between the DQN and the
grid-searched threshold baseline under the load spike, vs. an exact
tie or a 0.3-unit gap on `p99_latency`).

**What's still open, not a reward question:** the *size* of the
proactive-rekeying effect varied a lot by training seed (`0.256` vs.
`0.872`) under otherwise-identical config — worth a look (more seeds?
longer training?) before trusting this diagnostic's numbers as more
than "the mechanism clearly can produce proactive rekeying." Flagged
for discussion, not blocking.

**Concretely next:** build the real S4 scenario (or S2/S3, whichever
is more tractable first) — this likely needs the real tenant graph
(`build_tenant_graph`/`RequestGenerator`, still `NotImplementedError`)
for genuine per-tenant flooding, since today's load-spike diagnostic is
explicitly a cruder, tenant-blind stand-in, not real S4 itself. Do not
mistake `configs/default.yaml`'s new `load_spike:` block or
...
- [ ] 🚩 Gate W3 (make-or-break) — DQN beats the tuned threshold baseline on S1 and S3
      *(Still not attemptable for real — S3 doesn't exist as a scenario yet. A same-day
      2026-08-10 load-spike diagnostic (NOT real S3/S4 — see Next task and
      `configs/default.yaml`'s `load_spike:` block) showed `forced_rekey_ratio` dropping
      well below flat-S1's never-proactive `1.000` once arrival load genuinely varies,
      directionally confirming the reward mechanism isn't broken — but this is evidence
      toward attempting the gate for real once S3 exists, not the gate itself.)*
...
- **Date:** 2026-08-10
- **Commit:** `9a853a3` ("log: [solo] fix epsilon/training-step mismatch, re-run S1 comparison — 2026-08-10") — the commit this session started from; see SESSION_LOG.md for this session's own commit
- **`pytest` pass count:** 397 passed, 0 failed
