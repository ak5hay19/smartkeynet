...
"S1"` override — RESOLVED 2026-08-24 (this session), after being flagged
three separate times across three prior sessions (the multi-seed eval
probe, the S3 Gate W3 attempt's scratchpad workaround, and the S6 build
session) without being fixed.** `train()` now takes a real, explicit
`scenario: str = "S1"` parameter, following `experiments/harness.py`'s
`run_scenario`/`run_grid` convention exactly (an external override, not
read from `full_config["scenario"]`) — defaulting to `"S1"` so every
pre-existing call site is unaffected (proven byte-for-byte, not just
"tests pass" — see SESSION_LOG.md's newest entry). This also let the Hard
Rule 8 `train_eligible` guard finally be exercised through a real,
scenario-selectable end-to-end call (`scenario="S6"` against the real
`s6_migration.yaml`, confirmed to raise before any environment is even
constructed) for the first time — previously the guard only ever fired
because `train()` happened to ignore whatever scenario a config named, not
because it was genuinely reachable. This directly clears the way for the
soft-reward baseline agent's training work (below), which will need to run
on multiple scenarios. **Two standing, unrelated open items remain
flagged, not part of any thread**: `env/forecast_provider.py`'s placeholder
threat-feature formula's posture ceiling (below); and
...
- **Date:** 2026-08-24
- **Commit:** `588b0f7` ("feat: real S6 (migration wave) scenario dispatch, held-out-eval-only guard per Hard Rule 8 -- 2026-08-24") — the commit this session started from; see SESSION_LOG.md for this session's own commit
- **`pytest` pass count:** 506 passed, 1 xfailed (501 prior + 5 new in `tests/test_train.py` for `train()`'s new `scenario` parameter: the byte-for-byte S1-default before/after comparison, the S3 dispatch-proof pair, the S6 guard end-to-end proof, and the S3 control test)
- **Branch:** Confirmed `main`/`dev21` in sync at session start (`git rev-parse main dev21` both `588b0f7baa4c346f93f2deec8d7fb2b00ddc4b25`). This session's commit (real code: `experiments/train.py`'s `scenario` parameter, `tests/test_train.py`'s 5 new tests — nothing else touched, confirmed via `git status`) was ff-merged into `dev21` at the end — see SESSION_LOG.md for the final shared hash. Not pushed to origin this session, per instruction.
