...
**Thread 3: S4 is now real and dispatched (2026-08-24)** — the one remaining
named follow-up from that thread is **S6's scripted-schedule mechanism**,
still entirely unstarted (needs both tenant identity, which now exists, and
a new scripted-schedule config/dispatch mechanism, which doesn't) — see
Thread 3 below. This session's S4 result is **not directly relevant to
Gate W3's still-unresolved S3 half** (Thread 1) — S4 is about tenant-level
traffic fairness under a shared pool, not about QKD-degradation scarcity
visibility at default `pool:` scale, which is Thread 1's actual blocker;
stated explicitly here rather than left implicit, per instruction.
...
- **Date:** 2026-08-24
- **Commit:** `da4c925` ("feat: real NetworkX tenant graph + graph-driven RequestGenerator, Hard Rule 3 swap-tested -- 2026-08-23") — the commit this session started from; see SESSION_LOG.md for this session's own commit
- **`pytest` pass count:** 480 passed, 1 xfailed (469 prior + 5 new in `tests/test_environment.py`, up from 31 to 36 — incl. renaming the now-outdated `test_s4_and_s6_scenarios_are_not_yet_dispatched` to `test_s5_and_s6_...` + 6 new in `tests/test_request_generator.py`, up from 22 to 28)
- **Branch:** Confirmed `main`/`dev21` in sync at session start (`git rev-parse main dev21` both `da4c92554e1d32e1202c6f25402924a21023a5cc`). This session's commit (real code: `env/request_generator.py::RequestGenerator`'s `flood_override` mechanism, `env/environment.py`'s S4 dispatch (design decision 13), `configs/scenarios/s4_ddos.yaml`, plus tests and doc updates) was ff-merged into `dev21` at the end — see SESSION_LOG.md for the final shared hash. Not pushed to origin this session, per instruction.
