...
smoothed over; a tuned threshold's own comparable figures
have not yet been re-measured under the same configuration.
\end{abstract}
...
scenario. Table~\ref{tab:results}'s masked, soft-reward, and always-hybrid
rows are measured under the current S3 configuration described
below: means and standard deviations over 3 independently
trained checkpoints $\times$ 8 evaluation seeds each (24 evaluations)
for the first two, one held-out evaluation seed for the third.
Its remaining three rows (static threshold, always-PQC, uniform
random) are retained from an earlier run, predating a pool-capacity
recalibration described below, as ten-seed medians with
interquartile ranges; they have not yet been re-measured under
the current configuration, and a cross-row comparison involving
...
\begin{table*}[t]
\caption{Scenario S3 (QKD degradation). Exhaustion and regret are event counts; below-floor is the count of decisions served under the floor, the numerator of~(\ref{eq:belowfloor}). Rows marked \textdagger\ are a single held-out seed under the current configuration; rows marked \textdaggerdbl\ are retained from an earlier, pre-recalibration configuration and not yet re-measured (see Section~\ref{sec:impl})}
\label{tab:results}
\centering
...
Static threshold\textdaggerdbl & 150.0 & 42.0 & 42.0 & 6.9 & 0 \\
Always-hybrid\textdagger & 1.5000 & 18.0 & 18.0 & 8.1 & 0 \\
Always-PQC\textdaggerdbl & 150.0 & 531.0 & 531.0 & 95.5 & 0 \\
Random (safe set)\textdaggerdbl & 150.0 & 510.5 & 510.5 & 66.0 & 0 \\
\bottomrule
\end{tabular}
...
with the tuned threshold if one occurred. The threshold's own
figures in Table~\ref{tab:results} (marked \textdaggerdbl) come from the pool
configuration in place before Section~\ref{sec:impl}'s recalibration, and
have not yet been re-measured under the masked agent's current
configuration, so we cannot yet state a controlled tie-or-no-tie
result the way we can for the reward-shaped comparison above;
we flag the gap rather than compute a same-configuration
comparison the data do not support. What the threshold's own
past run showed, on its own terms, is that it reaches 42
pool-exhaustion events at a 6.9\% rekey ratio --- a real tradeoff
shape (buying availability with churn) worth re-confirming under
the current configuration before it is compared to the masked
agent again.

The static baselines behave as the scarcity argument predicts,
though only always-hybrid has been re-measured under the
current configuration: on a single held-out seed it drains the
pool and exhausts 18 times, against the masked agent's zero,
over the same 250-step episode (Table~\ref{tab:results}, marked \textdagger). Always-PQC's own past run is the more interesting case: it
never prefers pool material, yet still exhausted 531 times under
the earlier configuration, because masking forces it to hybrid
whenever a floor demands T2 and it has done nothing beforehand
to conserve. Non-anticipation, not appetite, is what exhausts
it; whether the magnitude holds under the current configuration
is, like the threshold's, not yet re-measured.
...
disqualification rule is really about, and that comparison remains
open until the threshold is re-measured under the current
configuration, per the previous subsection. We report the rekey
column regardless of which comparison it ultimately supports,
since a table carrying only regret and exhaustion would hide
the cost either way. The third named risk, the attack's
...
Under the current configuration, the masked agent and
always-hybrid coincide exactly at a p99 of 1.5000 despite an
exhaustion count that differs from zero to 18; the soft-reward
agent's own p99, 1.4064, is close but distinct. The same pattern
that motivated this caveat under the earlier configuration ---
where five of six policies agreed to one decimal place at 150.0,
and only the reward-shaped agent's 120.0 differed --- persists in
weaker form here: policies whose exhaustion counts differ
substantially should not agree this closely on a tail statistic.
The likely explanation is still a ceiling in the latency model or
the deferral timeout, in which case the column reports partial
saturation and not behavior. We draw no conclusion from it
until the cap is located; the three rows not yet re-measured
under the current configuration (Table~\ref{tab:results}, marked \textdaggerdbl)
still show the original 150.0 figure and have not been re-examined
for this artifact under the new configuration.
...
identity across six policies, holding under both the current
configuration's evaluation methodology and the earlier
configuration's ten-seed medians, means either that the counters
are wired to the same event, or that every S3 exhaustion happens
to produce exactly one deferral onset. We have not established
which, so for now the two columns should be read as one
...
than hidden. What remains, in order: locate the latency ceiling
and separate the exhaustion and regret counters, so Table~\ref{tab:results}
can be read at face value; re-measure the static threshold,
always-PQC, and random baselines under the current, recalibrated
configuration so the threshold comparison can be stated with
the same rigor as the reward-shaped one; and train the
