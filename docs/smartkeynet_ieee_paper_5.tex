...
masked agent records zero below-floor decisions against 1,012
(a 16.9\% below-floor rate) for a reward-shaped agent trained
on the same environment, and reaches zero pool-exhaustion
events against that same baseline's 3.54. Against that baseline
the masked agent's rekey churn is lower, not higher --- an
inversion of the trade we expected, reported rather than
smoothed over. A freshly tuned static threshold ties the masked
agent on every safety and availability column we measure, but
trails it by $\sim$3.8$\times$ on total episode reward, the objective
either is actually scored against, because it rekeys on almost
every decision rather than reusing a still-valid key.
\end{abstract}
...
the masked agent, the reward-shaped baseline, the four
non-RL baselines, and the adversarial trace generator of~(\ref{eq:steering})
are all implemented and run, and the results below and in
Section~\ref{sec:results}'s Limitations come from them. The learned
forecaster is the one exception: it remains a non-learned
placeholder, so those results exercise the masking and
budgeting machinery under a stub threat signal rather than a
trained one.
...
scenario. Every row of Table~\ref{tab:results} is now measured under
the current S3 configuration described below. The masked and
soft-reward rows report means and standard deviations over 3
independently trained checkpoints $\times$ 8 evaluation seeds each
(24 evaluations); always-hybrid is one held-out evaluation seed;
the remaining three (static threshold, always-PQC, uniform random)
have no training-seed dimension, being non-RL, so each is an
8-evaluation-seed mean under the same seeds (900--907) the masked
and soft-reward rows use. The static threshold is re-tuned on this
same configuration, not reused from before the recalibration: a
grid search over the same nine candidate thresholds, maximizing
mean total episode reward across the 8 eval seeds (not p99 latency,
which an earlier tuning pass found to be non-discriminating on this
grid), selects $t=0.9$, matching an earlier tuning run's own result
on this configuration exactly.
...
the masked DQN, the reward-shaped baseline, the four non-RL baselines, the adversarial trace generator of~(\ref{eq:steering}), and the
scenario comparison runner. This spine runs end to end on
S1 through S6, and Section~\ref{sec:results} reports S3; the S5 dose-response
sweep this generator drives is reported in Section~\ref{sec:results}'s
Limitations. The request stream samples a tenant graph rather
than an unstructured process. Partial: the forecaster has only
a non-learned exponentially-weighted-moving-average fallback
over a placeholder signal, so the dual-head model of Section~III-D
does not exist yet, and the numbers below therefore exercise
the masking and budgeting machinery under a stub threat
signal. Partial: the operator interface's decision-trace panel
(Fig.~\ref{fig:trace}) is implemented and rendered from real traces; the
remaining dashboard panels exist only as a hand-authored mockup
with fabricated values that we neither reproduce nor cite.
...
falls, and pool refill stops covering demand. Six policies, under
the mixed methodology Section~\ref{sec:impl} describes.

\begin{table*}[t]
\caption{Scenario S3 (QKD degradation). Exhaustion and regret are event counts; below-floor is the count of decisions served under the floor, the numerator of~(\ref{eq:belowfloor}). Rows marked \textdagger\ are a single held-out seed under the current configuration; rows marked \textsection\ are 8-evaluation-seed means under the current configuration (seeds 900--907, the same eval seeds the masked/soft-reward rows use), with no training-seed dimension, since these three are non-RL baselines with nothing to train}
\label{tab:results}
\centering
...
\midrule
Masked DQN (ours) & 1.5000 & 0.00 & 0.00 & 15.6 & 0 \\
Soft-reward DQN & 1.4064 & 3.54 & 3.54 & 70.3 & 1,012\footnotemark[2] \\
Static threshold (t=0.9)\textsection & 1.5000 & 0.00 & 0.00 & 8.0 & 0\footnotemark[3] \\
Always-hybrid\textdagger & 1.5000 & 18.0 & 18.0 & 8.1 & 0 \\
Always-PQC\textsection & 1.5000 & 0.00 & 0.00 & 8.0 & 0 \\
Random (safe set)\textsection & 1.5000 & 0.38 & 0.38 & 10.0 & 0 \\
\bottomrule
\end{tabular}

\footnotetext[1]{Simulator time units. See Section~\ref{sec:instrumentation} on the saturation visible in this column.}
\footnotetext[2]{A 16.9\% below-floor rate (1,012 of 6,000 tier-establishing decisions across the 24 evaluations).}
\footnotetext[3]{Ties the masked agent on every column shown here, but trails it by $\sim$3.8$\times$ on total episode reward (the scalar training itself optimizes, summed over the episode, not shown as a column): $-38{,}566.87$ against $-10{,}214.82$, reproducing an earlier tuning run's figure on this same configuration to two decimal places. See Section~\ref{sec:results}'s threshold-comparison discussion.}
\end{table*}
...
in where the threat signal enters, serves 1,012 decisions below
floor, a 16.9\% below-floor rate. Being precise about what that
does and does not show: it is not the steering attack, since no
adversary was present. It shows that a security term weighted
against latency and energy is traded away under ordinary
scarcity, before anyone tries to manipulate it. The attack of~(\ref{eq:steering})
makes that trade worse on purpose, and Section~\ref{sec:results}'s
Limitations reports it.

What the reward-shaped agent buys with those decisions is
smaller than we expected and not what the disqualification
framing in Section~\ref{sec:scenarios} anticipated: a marginally lower p99
(1.4064 against 1.5000) but a rekey ratio of 70.3\%, the highest
in the grid, not the lowest --- floor violations do not exempt it
from the same rekey costs every other policy pays. Under this
configuration the reward-shaped agent does not beat the masked
agent on any performance metric; its only edge is a small
latency gain, purchased with both below-floor service and the
grid's worst rekey churn. That the trade looks worse for the
unsafe design here, not better, is itself worth stating plainly
...
with the tuned threshold if one occurred, and under the current
configuration one does occur, on the columns Table~\ref{tab:results}
shows: re-tuned on this configuration by the same grid search used
before ($t=0.9$, Section~\ref{sec:impl}), the threshold ties the masked
agent on p99 latency, pool-exhaustion, regret, and below-floor
service, and edges it on forced-rekey ratio (8.0\% against
15.6\%). Taken at face value, that reads as a tuned threshold
matching or beating a trained agent on every metric the table
carries --- exactly the outcome the disqualification framing in
Section~\ref{sec:scenarios} names as a failure of the project's premise.
It is not the full picture, and we report the rest rather than let
the table's own column choice imply otherwise. Total episode reward,
the scalar actually being optimized, separates the two by
$\sim$3.8$\times$: $-10{,}214.82$ for the masked agent against
$-38{,}566.87$ for the tuned threshold (Table~\ref{tab:results},
footnote 3) --- the same margin, to two decimal places, an earlier
tuning run found on this identical configuration, which we take as
confirmation this is a stable measurement and not a seed artifact.
The mechanism is visible directly in the policy's own decision
rule: \texttt{StaticThresholdPolicy} never chooses to reuse an
existing session key, only to serve fresh SERVE\_HYBRID or SERVE\_PQC
(or the lowest legal tier) every single decision. Its low
forced-rekey ratio reflects that almost none of its many rekeys are
the \emph{forced} kind, not that it rekeys rarely --- a distinction
Table~\ref{tab:results}'s rekey column cannot show, since it counts
forced rekeys as a fraction of rekeys issued, not rekeys as a
fraction of decisions. The masked agent, which reuses a
still-valid, still-floor-clearing key whenever one is available,
pays that fresh-key cost far less often. So the threshold does not
match the agent on the objective either is actually evaluated
against; it matches it only on the subset of outcomes Table~\ref{tab:results}
happens to display, which is a narrower and more brittle claim than
the disqualification rule was written to guard against, and we say
so plainly rather than let the table stand for the whole comparison.

The static baselines mostly behave as the scarcity argument
predicts, though not uniformly, and the surprise is reported rather
than smoothed into the expected shape. On a single held-out seed,
always-hybrid drains the pool and exhausts 18 times against the
masked agent's zero, over the same 250-step episode
(Table~\ref{tab:results}, marked \textdagger) --- the clean case the
argument anticipates. Always-PQC and uniform random, both re-measured
under the current configuration for the first time this round, do
not show the large exhaustion counts an earlier, pre-recalibration
run reported (531 and 510.5 respectively): under the current pool,
always-PQC exhausts 0.00 times and random exhausts 0.38 times,
averaged over the same 8 eval seeds as the threshold. Non-anticipation
was the mechanism we expected to punish always-PQC --- it never
prefers pool material, but masking still forces it to hybrid
whenever a floor demands it, with nothing conserved beforehand --- and
that mechanism is real: its p99 latency still saturates at 1.5000
(Section~\ref{sec:instrumentation}), meaning it reaches SERVE\_HYBRID
often enough to hit that ceiling, the same way every other row does.
But under the current, smaller pool that forced draw is apparently
not sustained or concentrated enough within one 250-step eval episode
to exhaust it, unlike always-hybrid's policy of maximizing that same
draw on every single decision. We read this as a property of episode
length and draw concentration, not a reason to doubt the
recalibration: the always-hybrid row directly above, on the same
configuration, already shows that the current pool can be exhausted
under sufficient sustained demand --- 18 times on one seed --- so the
recalibration itself is not in question, only how much forced,
occasional hybrid draw it takes to reach that point.

\subsection{Failure modes: one materialized, one did not}

Section~\ref{sec:scenarios} named over-conservation as a risk, on the
grounds that a large $R_{\text{starve}}$ pushes toward hoarding. Relative
to the reward-shaped baseline it did not materialize: the
masked agent's 15.6\% rekey ratio is well below that baseline's
70.3\%, so its zero-below-floor, zero-exhaustion record is not
bought with more rekey churn than the design it is compared
against in Table~\ref{tab:results}. Whether it materializes relative to a
tuned threshold is the comparison Section~\ref{sec:scenarios}'s
disqualification rule is really about, and the previous subsection
now measures it under the current configuration. Read off the rekey
column alone, it looks like it does: the threshold's 8.0\% is lower
than the masked agent's 15.6\%. But that column is the fraction of
rekeys that were forced, not how often a rekey happens at all, and
the previous subsection's total-reward comparison is the more direct
test of hoarding: the masked agent's $-10{,}214.82$ against the
threshold's $-38{,}566.87$ says the opposite, since the threshold's
own decision rule pays a fresh-key cost on essentially every
decision and the masked agent's occasional REUSE does not. We report
the rekey column regardless of which reading it ultimately supports,
since a table carrying only regret and exhaustion would hide
the cost either way, and its answer here is why we look past it
rather than stop there. The third named risk, the attack's
sensitivity to the baseline's security weight specifically (as
opposed to attack strength $\alpha$, which the dose-response sweep
in this section's Limitations does cover), remains unrun.
...
Under the current configuration, five of the six rows now coincide
exactly at a p99 of 1.5000 --- masked, always-hybrid, the tuned
threshold, always-PQC, and random --- despite exhaustion counts
ranging from zero (masked, threshold, always-PQC) to 18
(always-hybrid); only the soft-reward agent's own p99, 1.4064, is
close but distinct. Every row is measured under the same
configuration now, which sharpens rather than weakens the caveat
first raised under the earlier configuration, where the same
five-of-six pattern held at a different absolute value (150.0):
policies whose exhaustion counts differ this substantially, from
zero to 18, should not agree this closely on a tail statistic, and
now every row that could confirm or break that pattern has, and
confirms it. The likely explanation is still a ceiling in the
latency model or the deferral timeout, in which case the column
reports partial saturation and not behavior. We draw no conclusion
...
identity across six policies, all now measured under the current
configuration's evaluation methodology, means either that the
counters are wired to the same event, or that every S3 exhaustion
happens to produce exactly one deferral onset. We have not
established which, so for now the two columns should be read as one
...
zero below-floor decisions against 1,012 for the reward-shaped
baseline, and lower rekey churn rather than higher against
that same baseline --- not the trade we expected, reported rather
than hidden. The threshold comparison Section~\ref{sec:scenarios}
committed us to is now stated with that same rigor: a freshly
tuned threshold ties the masked agent on every column
Table~\ref{tab:results} shows, but loses by $\sim$3.8$\times$ on
total episode reward, because it rekeys on nearly every decision
rather than reusing a still-valid key --- the disqualification
rule's premise survives, but only once the objective actually
being optimized, not just the table's own column choice, is
consulted. What remains, in order: locate the latency ceiling and
separate the exhaustion and regret counters, so Table~\ref{tab:results}
can be read at face value; and train the
dual-head forecaster on RT-IoT2022 to retire the moving-average
fallback. Longer term, deriving the floor table from compliance
